...
- Engine metadata remains broad (`cordova-android >=12`, `cordova-ios >=7`) so earlier supported platform releases are not intentionally dropped, but this refresh was smoke-tested against `cordova-android@14`/`cordova-android@15` and `cordova-ios@7`/`cordova-ios@8`.
